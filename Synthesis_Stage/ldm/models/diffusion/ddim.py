...
class DDIMSampler(object):
    ...
    @torch.no_grad()
    def ddim_sampling(self, cond, shape, delta_step=89,
        ...
        hijack_feat = None
        hijack_step = 0
        if "hijack_feat" in kwargs:
            hijack_feat = kwargs['hijack_feat']
            hijack_step = kwargs['hijack_step']
        ...
        for i, step in enumerate(iterator):
            ...
            if hijack_feat is not None and step > time_range[hijack_step]:
                continue
            ...
            outs = self.p_sample_ddim(img, cond, ts, delta_step=delta_step, index=index,
    ...
    @torch.no_grad()
    def p_sample_ddim(self, x, c, t, delta_step, index, repeat_noise=False, use_original_steps=False,
                      quantize_denoised=False, temperature=1., noise_dropout=0., score_corrector=None,
                      corrector_kwargs=None, unconditional_guidance_scale=1., unconditional_conditioning=None, **kwargs):
        ...
        if index == delta_step:
            delta, _ = self.model.apply_model(x, t, c, extract_mid=True)
            delta_x = x[:,:4,:,:] + delta
            x = torch.cat((delta_x, x[:,4:,:,:]), dim=1)
