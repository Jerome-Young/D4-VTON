...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        ...
        return x, inpaint, mask, reference, feat
...
class LatentDiffusion(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False, cond_key=None,
                  return_original_cond=False, bs=None, get_mask=False, get_reference=False, get_inpaint=False):

        # x, inpaint, mask, reference, feat = super().get_input(batch, k)
        x, inpaint, mask, reference, feat = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
        ...
        # z_diff = torch.cat([feat, z_inpaint, mask_resize], dim=1)
    ...
    def shared_step(self, batch, **kwargs):
        # x, x_diff, c, gt = self.get_input(batch, self.first_stage_key)
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.u_cond_prop < self.u_cond_percent:
            ...
            loss, loss_dict = self.p_losses(x, self.learnable_vector.repeat(x.shape[0], 1, 1), t, *args, **kwargs)
        else:
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, extract_mid=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            if extract_mid:
                delta, x_recon = self.model(x_noisy, t, extract_mid=extract_mid, **cond)
            else:
                x_recon = self.model(x_noisy, t, extract_mid=extract_mid, **cond)

        if isinstance(x_recon, tuple) and not return_ids:
            return x_recon[0]
        else:
            if extract_mid:
                return delta, x_recon
            else:
                return x_recon
    ...
    def sos(self, a, start_dim=1):  # sum of square
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        loss_simple = self.get_loss(output, target, mean=False).mean([1, 2, 3])
    ...
    def log_images(self, batch, N=4, n_row=4, sample=True, ddim_steps=100, ddim_eta=0., return_keys=None,
        ...
        z, c, x, xrec, xc, mask, reference, inpaint_image = self.get_input(batch, self.first_stage_key,
        ...
        if sample:
            ...
            x_samples = x_samples * (1 - mask) + x * mask
    ...
    def configure_optimizers(self):
        ...
        if self.cond_stage_trainable:
            ...
            params = params + list(self.cond_stage_model.final_ln.parameters()) + list(
                self.cond_stage_model.mapper.parameters()) + list(self.proj_out.parameters())
        if self.learn_logvar:
...
class DiffusionWrapper(pl.LightningModule):
    ...
    def forward(self, x, t, extract_mid=False, c_concat: list = None, c_crossattn: list = None):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, extract_mid=extract_mid)
        elif self.conditioning_key == 'concat':
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t, extract_mid=extract_mid)
        elif self.conditioning_key == 'crossattn':
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc, extract_mid=extract_mid)
        elif self.conditioning_key == 'hybrid':
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, extract_mid=extract_mid)
        elif self.conditioning_key == 'adm':
            cc = c_crossattn[0]
            out = self.diffusion_model(x, t, y=cc, extract_mid=extract_mid)
        else:
            raise NotImplementedError()
...
class LatentTryOnDiffusion(LatentDiffusion):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False, cond_key=None,
                  return_original_cond=False, bs=None, get_mask=False, get_reference=False, get_inpaint=False):
        x, inpaint, mask, reference, feat = DDPM.get_input(self, batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
            mask = mask[:bs]
        ...
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        encoder_warp = self.encode_first_stage(feat)
        z_warp = self.get_first_stage_encoding(encoder_warp).detach()
        encoder_posterior_inpaint = self.encode_first_stage(inpaint)
        z_inpaint = self.get_first_stage_encoding(encoder_posterior_inpaint).detach()
        mask_resize = Resize([z.shape[-2], z.shape[-1]])(mask)
        z_new = torch.cat((z, z_inpaint, mask_resize), dim=1)
        z_diff = torch.cat([z_warp, z_inpaint, mask_resize], dim=1)
    ...
    def shared_step(self, batch, **kwargs):
        z_tgt, z_src, c, x_tgt, inpaint_mask = self.get_input(batch, self.first_stage_key, get_mask=True)
        loss, loss_dict = self(z_tgt, c, z_src, x_tgt, inpaint_mask)
        return loss, loss_dict

    def forward(self, x, c, x_src, gt, mask, *args, **kwargs):
        self.opt.params = self.params
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        self.u_cond_prop = random.uniform(0, 1)
        if self.model.conditioning_key is not None:
        ...
        if self.u_cond_prop < self.u_cond_percent:
            self.opt.params = self.params_with_white
            # same noise
            loss, loss_dict = self.p_losses(x, self.learnable_vector.repeat(x.shape[0], 1, 1), t, noise)
            delta = self.estimate_delta(x_src, self.learnable_vector.repeat(x.shape[0], 1, 1), t, noise)
            x_pred = self.sample_hijack(x_src, self.learnable_vector.repeat(x.shape[0], 1, 1), t, delta, noise)
        else:
            # same noise
            loss, loss_dict = self.p_losses(x, c, t, noise)
            delta = self.estimate_delta(x_src, c, t, noise)
            x_pred = self.sample_hijack(x_src, c, t, delta, noise)

        loss_delta_weight = 5e-1
        z_noise = self.q_sample(x_start=x[:, :4, :, :], t=t, noise=noise)
        z_prime_noise = self.q_sample(x_start=x_src[:, :4, :, :], t=t, noise=noise)
        gt_delta = z_noise - z_prime_noise
        loss_delta = self.get_loss(delta, gt_delta.detach(), mean=True)
        loss += loss_delta * loss_delta_weight
        loss_dict.update({f'train/loss_delta': loss_delta * loss_delta_weight})
    ...
    def estimate_delta(self, x_start, cond, t, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start[:, :4, :, :]))
        x_noisy = self.q_sample(x_start=x_start[:, :4, :, :], t=t, noise=noise)
        x_noisy = torch.cat((x_noisy, x_start[:, 4:, :, :]), dim=1)
        delta_noise, _ = self.apply_model(x_noisy, t, cond, extract_mid=True)
        return delta_noise

    def sample_hijack(self, x_start, cond, t, delta, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start[:, :4, :, :]))
        x_noisy = self.q_sample(x_start=x_start[:, :4, :, :], t=t, noise=noise)
        x_noisy += delta
        x_noisy = torch.cat((x_noisy, x_start[:, 4:, :, :]), dim=1)
    ...
    def log_images(self, batch, N=4, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        data = self.get_input(batch, self.first_stage_key,
                              return_first_stage_outputs=True, force_c_encode=True, return_original_cond=True,
                              get_mask=True, get_reference=True, get_inpaint=True, bs=N)
        z, z_prime, c, x, xrec, warp, xc, mask, reference, inpaint_img = data
        N = min(x.shape[0], N)
        n_row = min(x.shape[0], n_row)
        log["inputs"] = x
        log["reconstruction"] = xrec
